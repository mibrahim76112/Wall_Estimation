...
It must be downloaded and placed manually, otherwise the backend will NOT run.

This backend is intended to be used by a website frontend (React, Next.js, etc.).
...
(from the CubiCasa pretrained model source or the Google Drive link provided by the project owner)
...
Ensure this line is disabled in:
...
```python
# model.init_weights()
If someone re-downloads CubiCasa again, this fix MUST be reapplied.
...
They are useful for QA and validation and can optionally be shown in the frontend.
...
Optional: display debug overlay images